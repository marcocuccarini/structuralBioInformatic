...
def Preprocessing(df):
    ...
    return df
def OneHotEncoder(df,y):
    pdb_idL = preprocessing.OneHotEncoder(sparse=False)
    pdb_idL.fit(df['pdb_id'])
    df['pdb_id']=pdb_idL.transform(df['pdb_id'])
    s_ins = preprocessing.OneHotEncoder(sparse=False)
    s_ins.fit(df['s_ins'])
    df['s_ins']=s_ins.transform(df['s_ins'])
    s_resn = preprocessing.OneHotEncoder(sparse=False)
    s_resn.fit(df['s_resn'])
    df['s_resn']=s_resn.transform(df['s_resn'])
    s_ch = preprocessing.OneHotEncoder(sparse=False)
    s_ch.fit(df['s_ch'])
    df['s_ch']=s_ch.transform(df['s_ch'])
    s_ss8    = preprocessing.OneHotEncoder(sparse=False)
    s_ss8.fit(df['s_ss8'])
    df['s_ss8']=s_ss8.transform(df['s_ss8'])
    s_ss3 = preprocessing.OneHotEncoder(sparse=False)
    s_ss3.fit(df['s_ss3'])
    df['s_ss3']=s_ss3.transform(df['s_ss3'])
    t_ch = preprocessing.OneHotEncoder(sparse=False)
    t_ch.fit(df['t_ch'])
    df['t_ch']=t_ch.transform(df['t_ch'])
    t_ch = preprocessing.OneHotEncoder(sparse=False)
    t_ch.fit(df['t_ch'])
    df['t_ch']=t_ch.transform(df['t_ch'])
    t_ins = preprocessing.OneHotEncoder(sparse=False)
    t_ins.fit(df['t_ins'])
    df['t_ins']=t_ins.transform(df['t_ins'])
    t_resn = preprocessing.OneHotEncoder(sparse=False)
    t_resn.fit(df['t_resn'])
    df['t_resn']=t_resn.transform(df['t_resn'])
    t_ss8 = preprocessing.OneHotEncoder(sparse=False)
    t_ss8.fit(df['t_ss8'])
    df['t_ss8']=t_ss8.transform(df['t_ss8'])
    t_ss3 = preprocessing.OneHotEncoder(sparse=False)
    t_ss3.fit(df['t_ss3'])
